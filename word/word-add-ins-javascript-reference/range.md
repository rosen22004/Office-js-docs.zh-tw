...
|[insertBreak(breakType: BreakType, insertLocation: InsertLocation)](#insertbreakbreaktype-breaktype-insertlocation-insertlocation)|void|在指定的位置插入中斷符號。除了換行符號可以插入至任何 body 物件，其他中斷符號只能插入到主文件內文所包含的 range 物件中。InsertLocation 值可以是 'Replace'、'Before' 或 'After'。|
...
在指定的位置插入中斷符號。除了換行符號可以插入至任何 body 物件，其他中斷符號只能插入到主文件內文所包含的 range 物件中。InsertLocation 值可以是 'Replace'、'Before' 或 'After'。
...
|insertLocation|InsertLocation|必要。此值可以是 'Replace'、'Before' 或 'After'。|
