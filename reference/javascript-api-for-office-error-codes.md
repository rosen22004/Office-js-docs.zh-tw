...
# <a name="javascript-api-for-office-error-codes"></a>適用於 Office 的 JavaScript API 錯誤碼
...
## <a name="binding-creation-error-conditions"></a>繫結建立錯誤狀況

在 API 中建立繫結時，指示您想要使用的繫結類型。下表列出繫結類型和預期的繫結行為結果。
...
|**指定繫結類型**|**實際選取範圍**|**行為**|
|:-----|:-----|:-----|
|Matrix|儲存格範圍 (包括資料表中，和單一儲存格)|在選取的儲存格上建立_矩陣_繫結類型。不需要修改文件。|
|矩陣|儲存格中選取的文字|在所有儲存格上建立_矩陣_繫結類型。不需要修改文件。|
|矩陣|多個選取範圍/無效選取範圍 (例如，使用者選取一張圖片、物件或文字藝術師)。|無法建立繫結。|
