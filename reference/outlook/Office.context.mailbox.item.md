...
對於未儲存至伺服器的項目，`itemId` 屬性會在撰寫模式下傳回 `null`。 如果項目識別碼為必要，[`saveAsync`](Office.context.mailbox.item.md#saveasyncoptions-callback) 方法可以用來將項目儲存至伺服器，其會將回呼函數中 [`AsyncResult.value`](simple-types.md#asyncresult) 參數內的項目識別碼傳回。
...
[ 和 `from`](Office.context.mailbox.item.md#from-emailaddressdetails)`sender` 屬性代表同一個人，除非郵件是由代理人所傳送。 在此情況下，`from` 屬性表示委派人，而寄件者屬性即表示代理人。
...
叫用時，這個方法會將目前郵件儲存為草稿，並透過回呼方法傳回項目識別碼。
