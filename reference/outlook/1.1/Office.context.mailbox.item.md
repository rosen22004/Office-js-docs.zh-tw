...
## [Office](Office.md)[.context](Office.context.md)[.mailbox](Office.context.mailbox.md). 項目
...
#### 附件：Array.<[AttachmentDetails](simple-types.md#attachmentdetails)>
...
#### 寄件者︰[EmailAddressDetails](simple-types.md#emailaddressdetails)
...
對於未儲存至伺服器的項目，`itemId` 屬性會在撰寫模式下傳回 `null`。 如果項目識別碼為必要，[`saveAsync`](Office.context.mailbox.item.md#saveAsync) 方法可以用來將項目儲存至伺服器，其會將回呼函數中 [`AsyncResult.value`](simple-types.md#asyncresult) 參數內的項目識別碼傳回。
...
下列程式碼會檢查項目識別碼是否存在。 如果 `itemId` 屬性傳回 `null` 或 `undefined`，其會將項目儲存至伺服器，並自非同步結果取得項目識別碼。
...
####  optionalAttendees：Array.<[EmailAddressDetails](simple-types.md#emailaddressdetails)>|[收件者](Recipients.md)
...
#### 召集人︰[EmailAddressDetails](simple-types.md#emailaddressdetails)
...
####  requiredAttendees：Array.<[EmailAddressDetails](simple-types.md#emailaddressdetails)>|[收件者](Recipients.md)
...
#### 資源：[EmailAddressDetails](simple-types.md#emailaddressdetails)
...
[ 和 `from`](Office.context.mailbox.item.md#from)`sender` 屬性代表同一個人，除非郵件是由代理人所傳送。 在此情況下，`from` 屬性表示委派人，而寄件者屬性即表示代理人。
...
#### getEntities() → {[實體](simple-types.md#entities)}
...
#### getEntitiesByType(entityType) → (可為 null) {Array.<(String|[Contact](simple-types.md#contact)|[MeetingSuggestion](simple-types.md#meetingsuggestion)|[PhoneNumber](simple-types.md#phonenumber)|[TaskSuggestion](simple-types.md#tasksuggestion))>}
...
#### getFilteredEntitiesByName(name) → (可為 null) {Array.<(String|[Contact](simple-types.md#contact)|[MeetingSuggestion](simple-types.md#meetingsuggestion)|[PhoneNumber](simple-types.md#phonenumber)|[TaskSuggestion](simple-types.md#tasksuggestion))>}
...
#### getRegExMatchesByName(name) → (可為 null) {Array.<String>}
