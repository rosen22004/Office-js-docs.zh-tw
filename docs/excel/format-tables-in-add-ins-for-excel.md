...
- 最初將格式設定之後，您可以使用此用途專屬的新方法之一來[清除或更新格式設定](#清除或更新格式設定)。
...
使用 `format:` 來指定您要套用到使用 `cells:` 定義為 JavaScript 機碼值組清單之範圍的格式設定。 如需支援的值清單，請參閱[支援的格式設定機碼和值](#支援的格式設定機碼和值)。
...
|**機碼**|**值**|**附註**|
|:-----|:-----|:-----|
| `style:`|"none" \| \<所有預先定義的表格樣式名稱\>|預先定義的表格樣式名稱：<br/><br/>"TableStyleLight1"、"TableStyleLight2"、"TableStyleLight3"、"TableStyleLight4"、"TableStyleLight5"、"TableStyleLight6"、
...
"TableStyleDark10"、"TableStyleDark11"<br/><br/>若要查看表格樣式的外觀，請在 Excel 中插入表格，在 [表格工具]**** \| [設計]**** 索引標籤上，選擇 [快速樣式]**** 下拉式功能表，然後選取預先定義的樣式。 樣式的工具提示會對應到上述清單中的其中一個值。|
| `headerRow:`|**布林值**||
| `firstColumn:`|**布林值**||
| `filterButton:`|**布林值**||
| `totalRow:`|**布林值**||
