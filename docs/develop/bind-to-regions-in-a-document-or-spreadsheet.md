...
### 使用 Bindings 物件的 getByIdAsync 方法，依識別碼取得繫結
...
### 使用 Office 物件的 select 方法，依識別碼取得繫結
...
[addHandlerAsync] 方法的第一個 `eventType` 參數會指定要訂閱的事件名稱。 [Office.EventType] 是可用的事件類型值的列舉。 `Office.EventType.BindingDataChanged 評估為字串 `"bindingDataChanged"`。
